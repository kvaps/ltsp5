...
update_time(GtkWidget * label)
{
    ...
    timestring = g_strdup_printf("%.2d.%.2d, %.2d:%.2d", timePtr->tm_mday,
                                 timePtr->tm_mon + 1, timePtr->tm_hour,
                                 timePtr->tm_min);
...
main(int argc, char *argv[])
{
    ...
    hoststring =
        g_strdup_printf("<b>%s (%s) //</b>", get_sysname(), get_ip());
